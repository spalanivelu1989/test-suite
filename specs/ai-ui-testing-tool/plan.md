...
├── .claude/skills/playwright-cli/   # Playwright CLI skill + references the agents use (C7)
...
**Removed (superseded by agents):** `src/crawler/*`, `src/flows/*`,
...
| ------------------------ | ------------------------------------------------------------------------- | -------------------- |
| Agent runtime            | Run one agent def via Agent SDK (browser = Playwright CLI), stream events | R6, R12              |
...
3. **Planner** — Agent SDK runs the planner def; it explores the live app with
   `npx playwright-cli` (open/snapshot/click/…) over `Bash` and `Write`s a Markdown
   plan into the workspace.
4. **Generator** — Agent SDK runs the generator def over the plan; it drives
   `npx playwright-cli` to verify each scenario and `Write`s the `.spec.ts` files
   into the workspace.
5. **Healer** — Agent SDK runs the healer def; it executes `npx playwright test`,
   inspects failures with `npx playwright-cli`, and repairs them; unfixable tests
   become `test.fixme()` with a comment.
...
- **Error paths:** invalid/unreachable URL → validation error + non-zero CI exit, no
  ...
  Playwright CLI / browser not installed → clear setup error.
...
| --- | --------------------------------------------------------------------- | ---------- | ------ | ---------------------------------------------------------------------------------------------- |
| RK1 | Agent + browser runs are slow/expensive (3 agent loops + CLI browser) | High       | Med    | Bound crawl/plan scope; document cost; reuse v1's small caps for tests/demo                    |
