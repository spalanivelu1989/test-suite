...
the tool runs **four sequential agents** — **Discoverer → Designer → Tester →
...
runnable Playwright `.spec.ts` files (grounded against a live browser); the Tester
...
Tester (the pipeline becomes **Discoverer → Designer → Validator → run → Tester →
...
never blocks the run) and its findings are **fed to the Tester** so flagged
...
| R12 | Organize the pipeline as **four sequential agents** — Discoverer → Designer → Tester → Reporter — each a single responsibility, with a deterministic **Validation stage (R18)** inserted between Designer and Tester.                                                                                                                                                                                                                                                                                                                                                                    | Must     |
...
| R15 | The **Tester** runs the suite, repairs failing tests and re-runs to green; tests it cannot fix are marked `test.fixme()` with an explanatory comment — never reported as passed.                                                                                                                                                                                                                                                                                                                                                                                                          | Must     |
...
| R20 | Validation is **advisory, never a hard gate** — the suite still runs, heals, and reports regardless of findings — **and** the findings are **fed to the Tester** (appended to its prompt) so it repairs flagged anti-patterns alongside runtime failures.                                                                                                                                                                                                                                                                                                                                 | Must     |
...
| SC3  | A previously-passing test's locator no longer matches after a UI change                                                          | The Tester detects the failure, attempts an AI repair, re-runs, and the report states whether healing succeeded.                                                                 | R9,R15                |
| SC4  | The Tester cannot fix a genuinely failing test after repair attempts                                                            | The Tester marks it `test.fixme()` with an explanatory comment; the report shows it as needs-attention, not passed.                                                              | R15,R16               |
...
| SC13 | A generated spec uses `waitForTimeout`, `networkidle`, or a positional `.nth(n)` / xpath / deep-CSS selector                     | The Validator records `hard-wait` / `networkidle` / `brittle-selector` warnings; these are appended to the Tester's prompt; the suite still runs, heals, and reports (advisory). | R18,R20               |
...
- **Primary journey:** User enters a URL → watches the stages stream
  ...
  Tester running/repairing → Reporter aggregating) → reads a rich report,
...
| C7  | Adopt the **Playwright Agents** architecture: discoverer/designer/tester/reporter agents using `playwright-cli` (npx playwright-cli) for browser actions (ref: the agent defs in `/Users/senthilpalanivelu/Downloads/test/.claude/agents`).                                            |
...
| A4  | **Validation findings help, not hurt, the Tester.** Feeding anti-pattern findings into the Tester's prompt improves the repaired suite and does not cause the Tester to "fix" intentional test logic. The Tester is instructed to address findings only where they don't change intent.      |
...
| M3  | Auto-heal success rate               | 33% (v0.1.0 keyed run) | **≥50%** healed | % of locator-failure cases the Tester repairs and re-runs to green                                                                                              |
...
| AC7  | In a simulated locator break, the Tester attempts a repair and the report states whether healing worked.                                                                                                                                       | R9       |
...
| AC12 | A run executes the four agents in order Discoverer→Designer→Tester→Reporter (observable in progress events/logs).                                                                                                                              | R12      |
...
| AC15 | A test the Tester cannot fix is marked `test.fixme()` with a comment and shown as needs-attention (never as passed).                                                                                                                           | R15      |
...
| AC18 | A run executes the stages in order Discoverer→Designer→**Validator**→Tester→Reporter, with a `validating` progress event (carrying score + error/warning counts) observable in the events/logs between generating and running.                 | R18,R21  |
...
| AC23 | The Validator's findings are appended to the Tester's prompt (observable in the prompt text), and the suite still runs, heals, and reports regardless of findings (validation never blocks the run).                                           | R20      |
...
- **Validation as a hard gate** — the Validator never blocks the run or
  auto-quarantines specs; it is advisory + Tester-fed only (R20).
...
| Q11 | Should validation ever become a **hard gate** (fail the run / quarantine specs on error-level findings)?                              | Deferred — advisory + Tester-fed by decision (R20); revisit if low-value specs still slip through (Future vision).      |
...
| v0.3.0  | 2026-06-05 | Add deterministic **Validation stage** between Designer and Tester (R18–R21; SC11–SC15; AC18–AC24); constraint C8 (deterministic, no browser/LLM); metric M4 (generated-test quality); assumptions A1–A4; note R12 inserts the Validator. Advisory + Tester-fed, not a hard gate                                                                                                                     | Reconcile a feature built ahead of the Spec back into the contract — make generated-test _quality_ (assertions, robustness, relevance), not just pass/fail, observable and acted on |
| v0.3.1  | 2026-06-13 | **Terminology only** — rename the three agents Planner→**Discoverer**, Generator→**Designer**, Healer→**Tester** throughout the contract, code, and agent definition files. No behaviour change. The knowledge-base "healing" data vocabulary (`healing_events`, `HealingEvent`, heal precedents) and the persisted `RunStage` values (`planning`/`generating`/`healing`) are intentionally unchanged | User-requested rename; the heal-data layer is shipped on `main`, so renaming it was out of scope to avoid a schema migration                                                        |
