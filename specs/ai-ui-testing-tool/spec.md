...
**v0.3.0 adds a deterministic Validation stage** between the Generator and the
Healer (the pipeline becomes **Planner → Generator → Validator → run → Healer →
...
never blocks the run) and its findings are **fed to the Healer** so flagged
...
| R12 | Organize the pipeline as **four sequential agents** — Planner → Generator → Healer → Reporter — each a single responsibility, with a deterministic **Validation stage (R18)** inserted between Generator and Healer.                                                                                                                                                                                                                                                                                                                                                                       | Must     |
...
| R20 | Validation is **advisory, never a hard gate** — the suite still runs, heals, and reports regardless of findings — **and** the findings are **fed to the Healer** (appended to its prompt) so it repairs flagged anti-patterns alongside runtime failures.                                                                                                                                                                                                                                                                                                                                  | Must     |
...
| SC11 | The Generator finishes writing specs                                                                                             | The Validator runs next; a `validating` progress event reports the suite score and error/warning counts before the run/heal stages.                                              | R18,R21               |
...
| SC13 | A generated spec uses `waitForTimeout`, `networkidle`, or a positional `.nth(n)` / xpath / deep-CSS selector                     | The Validator records `hard-wait` / `networkidle` / `brittle-selector` warnings; these are appended to the Healer's prompt; the suite still runs, heals, and reports (advisory). | R18,R20               |
...
- **Primary journey:** User enters a URL → watches the stages stream
  (Planner exploring → Generator writing specs → **Validator checking specs** →
  Healer running/repairing → Reporter aggregating) → reads a rich report,
...
| A4  | **Validation findings help, not hurt, the Healer.** Feeding anti-pattern findings into the Healer's prompt improves the repaired suite and does not cause the Healer to "fix" intentional test logic. The Healer is instructed to address findings only where they don't change intent.          |
...
| AC18 | A run executes the stages in order Planner→Generator→**Validator**→Healer→Reporter, with a `validating` progress event (carrying score + error/warning counts) observable in the events/logs between generating and running.                    | R18,R21  |
...
| AC23 | The Validator's findings are appended to the Healer's prompt (observable in the prompt text), and the suite still runs, heals, and reports regardless of findings (validation never blocks the run).                                            | R20      |
...
- **Validation as a hard gate** — the Validator never blocks the run or
  auto-quarantines specs; it is advisory + Healer-fed only (R20).
...
| Q11 | Should validation ever become a **hard gate** (fail the run / quarantine specs on error-level findings)?                              | Deferred — advisory + Healer-fed by decision (R20); revisit if low-value specs still slip through (Future vision).       |
...
| v0.3.0  | 2026-06-05 | Add deterministic **Validation stage** between Generator and Healer (R18–R21; SC11–SC15; AC18–AC24); constraint C8 (deterministic, no browser/LLM); metric M4 (generated-test quality); assumptions A1–A4; note R12 inserts the Validator. Advisory + Healer-fed, not a hard gate | Reconcile a feature built ahead of the Spec back into the contract — make generated-test _quality_ (assertions, robustness, relevance), not just pass/fail, observable and acted on |
