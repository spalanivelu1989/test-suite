...
sequenceDiagram
    ...
    A2->>A2: Write brand-new scripts for everything not yet covered
    end

    rect rgb(255, 243, 224)
    Note over A3,PW: STEP 3 — Run the tests for real, then self-heal
...
| **Where is Playwright (the real browser) used?**               | In **two places** — the **Discoverer** drives it to explore the site, and the tests are **run** in it before the **Evolver** repairs failures.                                                                                                                     |
| **What does each agent do?**                                   | **Discoverer** → explore + plan · **Designer** → write the tests (reusing past ones) · **Evolver** → run + fix · **Reporter** → explain + store for next time.                                                                                                     |
