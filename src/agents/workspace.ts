...
/** Custom Playwright reporter that streams one tagged line per finished test. */
const REPORTER_FILE = "spec-stream.cjs";
/** Line prefix the streaming reporter emits; parsed back out in runSuiteAt. */
const SPEC_MARKER = "@@SPEC@@";

/** One finished test, streamed live from the running suite (vs. the end-of-run JSON). */
export interface SpecEvent {
  /** The test's title (matches the JSON report's spec title). */
  title: string;
  /** The spec filename (basename), e.g. "contact-section.spec.ts". */
  file: string;
  /** Playwright's terminal status for this test. */
...
/**
 * A Playwright reporter (CommonJS so the test subprocess needs no transpile) that
 * writes a single machine-readable line to stdout as each test finishes. The
 * built-in json reporter only writes at the very end, so this is what lets the run
 * feed show per-spec progress live against the target site. runSuiteAt parses
 * these `@@SPEC@@{…}` lines; everything else on stdout is ignored.
 */
const SPEC_STREAM_REPORTER = `// AUTO-GENERATED by the test harness — do not edit.
class SpecStreamReporter {
  onTestEnd(test, result) {
    try {
      const file = String((test.location && test.location.file) || '').split('/').pop() || '';
      const payload = { title: test.title, file, status: result.status, durationMs: result.duration };
      process.stdout.write('${SPEC_MARKER}' + JSON.stringify(payload) + '\\n');
    } catch (_) { /* never let reporting break the run */ }
  }
}
module.exports = SpecStreamReporter;
`;
/**
...
export interface Workspace {
  ...
  /**
   * Run the generated suite in this workspace and return Playwright's raw JSON
   * report. `onSpec` (optional) fires once per test as it finishes, for live
   * per-spec progress; the returned report is still the authoritative end state.
   */
  runSuite(
    signal?: AbortSignal,
...
export default defineConfig({
  ...
  reporter: [['json', { outputFile: '${RESULTS_FILE}' }], ['./${REPORTER_FILE}']],
  use: {
    headless: true,
...
async function runSuiteAt(
  ...
  await new Promise<void>((resolve) => {
    ...
    // Parse the streaming reporter's `@@SPEC@@{…}` lines as they arrive so the run
    // feed can show each spec's result live. Buffer partial lines across chunks.
    let buf = "";
    child.stdout.on("data", (chunk) => {
      ...
      while ((nl = buf.indexOf("\n")) !== -1) {
        ...
        buf = buf.slice(nl + 1);
        if (!line.startsWith(SPEC_MARKER)) continue;
        try {
          onSpec(JSON.parse(line.slice(SPEC_MARKER.length)) as SpecEvent);
        } catch {
          /* ignore a malformed/truncated line */
        }
      }
    });
...
export async function createWorkspace(
  ...
  // Per-spec live streaming reporter (see SPEC_STREAM_REPORTER). The suite config
  // references it by relative path, so it must exist before the suite runs.
  await writeFile(join(root, REPORTER_FILE), SPEC_STREAM_REPORTER, "utf8");
  ...
  return {
    ...
    configPath,
    authStatePath,
    runSuite: (signal?: AbortSignal, onSpec?: (e: SpecEvent) => void) =>
      runSuiteAt(root, signal, onSpec),
