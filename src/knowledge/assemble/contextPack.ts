import type { CoverageDecision, DesignerPack, SpecRef } from "../types";

// Pure assembly of the Designer's context pack (Spec R10/N4, Plan D7). The
// service decides cold/warm/KB-down BEFORE calling this; only the Designer
// receives a pack — the Discoverer is KB-agnostic (see ADR-0003), so there is no
// discoverer pack.
//
//   generating → warm: buildDesignerPack(decisions, specs) | cold/down: {}
//
// `specs[].code` is the copy-forward payload: a `reuse` spec is written verbatim
// to the tests dir (tagged @kp-reused) and is NEVER injected into the Designer
...
export function buildDesignerPack(
