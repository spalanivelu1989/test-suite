...
export async function discoverTests(
  ...
  const prompt =
    ...
    [
      ...
      "user flows (navigation, key content pages, forms, search).",
      "For every scenario, list the COMPLETE step sequence from a fresh page load to each assertion — never assume a section is visible by default. If an element is revealed only after an interaction (an '(optional)' toggle, mode selector, accordion, or tab), include that reveal step explicitly before any step that verifies or fills it.",
...
export async function designTests(
  ...
  const prompt = [
    ...
    "(1) explore it with playwright-cli via Bash from a FRESH page state (re-open the URL so toggles/inputs from a previous scenario do not leak), then",
    "(2) IMMEDIATELY use the Write tool to save its spec before moving to the next scenario.",
    "Do not skip a single step: for every element a step verifies or fills, snapshot to confirm it is actually visible from the fresh state — if it is gated behind an '(optional)' toggle, mode selector, accordion, or tab, add the explicit interaction that reveals it as a step BEFORE the assertion, even when the plan omitted it. The generated spec must reproduce the whole path from a blank page on its own.",
